...
```{r}
ggplot(d_faces, aes(x = response, fill = face_age)) + 
...
summary(m_1)
```
Ok. When predict responses, the face_type (1 or 2) is not a significant predictor. Good new
...
m_2_bayesian <- stan_glm(
  ...
  prior = lognormal_lpdf(0,1, autoscale = TRUE),
...
```{r}
posterior_interval(m_2_bayesian, prob = 0.80)
```
