...
# res[[4]] are the data of the dot numeric estimation for reference of subject 1
...
# res[[8]] are the data of the dot numeric estimation for reference of subject 2
  ...
  if(condition1 & condition2 ){ # new participant
    if(!(res[[s]][ind_prolific]$prolific %in% prolific)){
      ...
      if(s<3){
        ...
        estimation_reference_pos1 <- c(estimation_reference_pos1, df_exp$response[[35]]$Estimation)      
        df_exp2 <- as.data.frame(res[[4]])
        estimation_reference_pos2 <- c(estimation_reference_pos2, df_exp2$response[[35]]$Estimation)
        }else{
          df_exp <- as.data.frame(res[[s+1]])
          estimation_reference_pos1 <- c(estimation_reference_pos1, df_exp$response[[35]]$Estimation)
          
          df_exp2 <- as.data.frame(res[[s+3]]) 
          estimation_reference_pos2 <- c(estimation_reference_pos2, df_exp2$response[[35]]$Estimation)
        }
      ...
      if(length(res)-s >= 5 ){
        ...
        if(is.null(res[[s+6]]$Problems) ==FALSE){
          Problems <- c(Problems, res[[s+6]]$Problems)
        }else{
...
type <- c(rep("many",length(estimation_reference_pos1)) , rep("many",length(estimation_reference_pos2)))
estimation <- c(estimation_reference_pos1, estimation_reference_pos2)
