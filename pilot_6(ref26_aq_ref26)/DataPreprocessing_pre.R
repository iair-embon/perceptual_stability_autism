...
# data
content<-readLines(root$find_file("pilot_6(ref26_aq_ref26)/jatos_results_20240403143538.txt"))
res<-lapply(content,fromJSON)

# load the function to read the .txt results from JATOS and create a dataframe
source(root$find_file("pilot_6(ref26_aq_ref26)/initial_df_pilot.R"))
...
write.csv(df_exp, "pilot_6(ref26_aq_ref26)/df_exp.csv", row.names=FALSE)
write.csv(df_NotExperimentData, "pilot_6(ref26_aq_ref26)/df_NotExperimentData.csv", row.names=FALSE)
